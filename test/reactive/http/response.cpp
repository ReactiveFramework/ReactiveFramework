...
BOOST_AUTO_TEST_CASE(test_response_setget)
{
    ...
    BOOST_CHECK_EQUAL(response.getHeaders().size(), 1U);
    ...
    BOOST_CHECK_EQUAL(response.getHeaders().size(), 2U);

    response.getHeaders().add("X-Transaction", "260fd0ee77f9f784");

    BOOST_CHECK_EQUAL(response.getHeaders().size(), 3U);

    BOOST_CHECK_EQUAL(response.getCookies().size(), 0U);

    response.addCookie("foo", "bar");

    BOOST_CHECK_EQUAL(response.getCookies().size(), 1U);
}
...
BOOST_AUTO_TEST_CASE(test_parse_response)
{
    ...
    BOOST_CHECK_EQUAL(response.getHeaders().size(), 4U);
